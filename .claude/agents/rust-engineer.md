...
Panic-free code requirements for this project:
...
- `src/controller/reconciler.rs`: Main reconciliation loop
- `src/controller/state_machine.rs`: Formal FSM for resource lifecycle
- `src/controller/context.rs`: Shared context with Kubernetes client
- Multiple controllers can run concurrently in main.rs with shared Context
